function invesSearchController() {
	var ctrl = this;
	...
	ctrl.processSearch = function(term){
		...
		angular.forEach(ctrl.data, function(item) {
		    ...
		    if(item[ctrl.displayProperty].toUpperCase().indexOf(term) !=-1 ){
		    	...
		    	if(isNaN(Number(ctrl.bindType))){
		    		if(item[ctrl.bindType] != undefined){
				    	obj[ctrl.bindTypeProperty] = item[ctrl.bindType];
		    		}
		    	} else {
	...
	ctrl.traverseData = function(key) {
		if(ctrl.keys.upArrow == key) {
			...
			let obj = {};
			obj[ctrl.bindItemNoProperty] = (ctrl.itemNoProperty == undefined ? "": 0);
			obj[ctrl.bindTypeProperty] = ctrl.bindType;
...
app.component('investigationSearchComponent', {
    templateUrl : 'app/components/commonComponents/investigation-search-component/investigationSearchComponent.html',
    controller : invesSearchController,
    bindings : {
      	data: '=',
      	place: '@',
		onSelect: '&',
